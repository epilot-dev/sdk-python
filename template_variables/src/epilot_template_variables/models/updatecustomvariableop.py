"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .customvariable import CustomVariableInput, CustomVariableInputTypedDict
from epilot_template_variables.types import BaseModel
from epilot_template_variables.utils import (
    FieldMetadata,
    PathParamMetadata,
    RequestMetadata,
)
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class UpdateCustomVariableRequestTypedDict(TypedDict):
    id: str
    r"""Custom variable ID"""
    custom_variable: NotRequired[CustomVariableInputTypedDict]


class UpdateCustomVariableRequest(BaseModel):
    id: Annotated[
        str, FieldMetadata(path=PathParamMetadata(style="simple", explode=False))
    ]
    r"""Custom variable ID"""

    custom_variable: Annotated[
        Optional[CustomVariableInput],
        FieldMetadata(request=RequestMetadata(media_type="application/json")),
    ] = None
